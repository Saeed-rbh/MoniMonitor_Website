...
import CircularProgressBar from "@/AddTransaction/Notif/CircularProgressBar";
...
const Notification = ({
  ...
  useEffect(() => {
    setCloseData([
      open === "delete" ? "Transaction Deleted" : "Successfully Modified",
      open === "delete" ? (
  ...
  const [openStyle, api] = useSpring(() => ({
    ...
    outline:
      open === "delete" ? "1px solid var(--Ac-2)" : "1px solid var(--Gc-2)",
  }));
  ...
  const HandleDelete = () => {
    ...
    api.start({
      height: 55,
      border:
        open === "delete" ? "1px solid var(--Gc-2)" : "1px solid var(--Fc-2)",
    });
    setTimeout(() => {
      setDeleted(false);
      api.start({
