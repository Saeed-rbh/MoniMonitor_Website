...
//     // Check if the user has scrolled too far up or down, and reset scroll position
...
const ScrollableList = ({ item, items, onSelect, onConfirm }) => {
  ...
  useEffect(() => {
    if (onConfirm) {
      onSelect(currentIndex % items.length);
    }
  }, [currentIndex, onConfirm, onSelect, items.length]);
